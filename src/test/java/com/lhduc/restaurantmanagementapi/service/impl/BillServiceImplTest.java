...
class BillServiceImplTest {
    ...
    void testAddMoreBillItems() {
        ...
        List<BillDetailCreateRequest> billDetailCreateRequests = this.prepareBillDetailCreateRequests(menuItemId, menuItemQuantity);

        billService.addMoreBillItems(billId, billDetailCreateRequests);

        BillDto billAfterAddMoreItems = billService.getBillById(billId);
        assertEquals(1, billAfterAddMoreItems.getDetails().size());
        assertEquals(menuItemQuantity, billAfterAddMoreItems.getDetails().get(0).getQuantity());
    }
